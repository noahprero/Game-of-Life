...
const config = {
  rows: 75,
  cols: 100,
  defaultBuildCooldown: null, // iterations
  actualSimSpeed: 0, // ms
  visualSimSpeed: 10
};
...
let currentBuildSelection = "glider";
...
const gallery = document.getElementsByClassName("gallery-image");
for(let i = 0; i < gallery.length; i++) {
  gallery[i].addEventListener(("click"), function() {
    if(currentBuildSelection == this.id){
      return;
    }

    this.style.border = "4px solid white";
    document.getElementById(currentBuildSelection).style.border = "4px solid transparent";
    currentBuildSelection = this.id;
  });
}
...
function Build(row, col){
  ...
  if(currentBuildSelection == "glider"){
  ...
  else if(currentBuildSelection == "brush"){
  ...
  else if(currentBuildSelection == "rocket"){
  ...
  else if(currentBuildSelection == "minirocket"){
    tempArray[row][col + 4] = 1;
    tempArray[row + 1][col+ 4] = 1;
    tempArray[row - 1][col + 5] = 3;
    tempArray[row][col + 6] = 3;
  }
  else if(currentBuildSelection == "rpentomino"){
  ...
  else if(currentBuildSelection == "starslug"){
